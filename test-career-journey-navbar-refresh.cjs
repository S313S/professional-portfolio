...
(async () => {
  ...
  const beforeReload = await getNavbarSnapshot(page, 'before-reload');
  logSnapshot(beforeReload);
  assertNavbarHidden(beforeReload);
  ...
  const afterReload = await getNavbarSnapshot(page, 'after-reload');
  logSnapshot(afterReload);
  assertNavbarHidden(afterReload);
